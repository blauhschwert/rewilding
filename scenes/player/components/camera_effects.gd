...
func calculate_view_offset(delta : float) -> void:
	...
	if enable_tilt:
		...
		var forward_dot = velocity.dot(foward)
	...
	# Fall Kick
	if enable_fall_kick:
		var full_ratio = max(0.0, _fall_timer / fall_time)
		var fall_kick_amount = full_ratio * _fall_value
	...
	if enable_damage_kick:
		...
		# damage_ratio = ease(daage_ratio, -2) # If you want to ease over time
...
func add_damge_kick(pitch: float, roll: float, source: Vector3):
